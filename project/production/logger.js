let onLoaded, floatui;
ui.post(() => {
    let x, y, windowY, windowX, logWidth = 200, logHeight = 270, tabHeight = 25;
    floatui = floaty.window(`
                <vertical>
                    <horizontal w="${logWidth}" h="${tabHeight}" bg="#88888888">
                        ...
                        <vertical gravity="center" w="${logWidth - 2 * tabHeight}" h="${tabHeight}">
                            <text text="日志" w="*" h="*" gravity="center" textColor="#ffffff"/>
                        </vertical>
    ...
    floatui.web.getSettings().setAllowFileAccessFromFileURLs(!0);
    floatui.web.getSettings().setAllowUniversalAccessFromFileURLs(!0);
    floatui.web.loadUrl("file://" + files.path("./dist/index.html"));
    ...
    floatui.web.jsBridge.registerHandler("onLoaded", (d, call)=>{
        log(d)
        onLoaded && onLoaded()
        floatui.web.jsBridge.callHandler("home", "", ()=> {})
        call()
    })
